...
with tab1:
    ...
    with st.sidebar:
        ...
        v1 = st.number_input("Vibración Inicial (V1)", value=None, placeholder="Ej: 3.0 mm/s", key="v1_val")
        ...
        for i in range(2, 5):
            ...
            ang_def = float((i-2)*120.0)
            a = st.number_input(f"Ángulo V{i} (°)", value=ang_def, key=f"a{i}_val")
    ...
    if st.button("⚖️ CALCULAR BALANCEO Y GENERAR PDF", type="primary", use_container_width=True):
        ...
        if errores:
            ...
            # Intersecciones entre pares de círculos
            ...
            if i12 and i23 and i31:
                ...
                for i in range(3):
                    ax.add_patch(plt.Circle(centros[i], meds[i]['v'], fill=False, color='#3B82F6', alpha=0.4, ls='--'))
                ax.add_patch(plt.Polygon(mejor_tri, color='#FDE047', alpha=0.6, label="Área de Error"))
                ...
                for e in range(6):
                    ...
                    ax.text(-lim_max*1.05*math.sin(ang_e), lim_max*1.05*math.cos(ang_e), f"{e*72}°", ha='center', fontsize=9)
                
                ax.set_aspect('equal'); ax.set_xlim(-lim_max, lim_max); ax.set_ylim(-lim_max, lim_max)
                ax.grid(True, alpha=0.2); ax.axhline(0, color='black', lw=1); ax.axvline(0, color='black', lw=1)
                st.pyplot(fig)
                ...
                def export_pdf():
                    ...
                    pdf.cell(63, 7, "Punto", 1); pdf.cell(63, 7, "Vibración (mm/s)", 1); pdf.cell(64, 7, "Angulo (°)", 1, ln=True)
                    pdf.set_font("Arial", "", 10)
                    for i, m in enumerate(meds, 2):
                        pdf.cell(63, 7, f"V{i}", 1); pdf.cell(63, 7, str(m['v']), 1); pdf.cell(64, 7, f"{m['a']}°", 1, ln=True)
                    ...
                    pdf.cell(100, 7, "Peso Total de Corrección:", 1); pdf.cell(90, 7, f"{round(peso_total, 2)} g", 1, ln=True)
                ...
                st.error("❌ Los datos ingresados no permiten encontrar una intersección. Verifique las lecturas.")
...
with tab2:
    ...
    **PROTOCOLO DE SEGURIDAD OBLIGATORIO:**
    1. **Comunicación Eléctrica:** Informar a Planta Eléctrica que se iniciará el balanceo.
    2. **Gestión de Sensores:** Notificar a Panel para **CONGELAR LOS SENSORES** de vibración.
    3. **Bloqueo (LOTO):** El equipo debe estar **TOTALMENTE APAGADO Y BLOQUEADO** al manipular los pesos de prueba.
    """)
    st.markdown("""
    ### Pasos Técnicos:
    * **Paso 1:** Medir vibración inicial ($V_1$).
    * **Paso 2:** Colocar peso de prueba sucesivamente en 0°, 120° y 240° ($V_2, V_3, V_4$).
    * **Paso 3:** Ingresar datos para obtener el vector de corrección y su distribución en los eyectores a 72°.
    """)
